...
SELECT CONCAT(first_name, ' ', last_name) AS full_name FROM employees WHERE first_name LIKE 'E%e';

--Convert the names produced in your last query to all uppercase.
SELECT UPPER(CONCAT(first_name, ' ', last_name)) AS full_name FROM employees WHERE first_name LIKE 'E%e';
...
SELECT first_name, last_name, hire_date, DATEDIFF(CURDATE(), hire_date) AS days_at_company FROM employees WHERE hire_date LIKE '199%' AND birth_date LIKE '%12-25';

-- Find the smallest and largest current salary from the salaries table.
SELECT MAX(salary) FROM salaries WHERE to_date > CURDATE(); -- Max is 158,220 / Min is 36,623
